...
export class Jobs {
  ...
  async poll() {
    ...
    for (let device of this.devices) {
      ...
      if (jobbossId) {
        ...
        if (jobs === undefined) continue // must have had a problem - skip this device
  ...
  async getJobs(jobbossId) {
    ...
    try {
      ...
      //. what if recordset is empty? then would get []? fix
  ...
  handleJobs(device, jobs) {
    ...
    this.cache.set(`${device.id}-job`, activeJob)
