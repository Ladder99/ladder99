...
async function main() {
  ...
  for (const device of devices) {
    ...
    tcp.on('connection', async socket => {
      ...
      //. iterate over sources, call setSocket(source.outputs, socket) or sthing
      // cache.setSocket(socket)
      ...
      for (const source of device.sources) {
        ...
        if (outputTemplates) {
          ...
          //. will want to save those outputs onto the source object,
          // so can call setSocket later
    ...
    // agent will try to connect here.
